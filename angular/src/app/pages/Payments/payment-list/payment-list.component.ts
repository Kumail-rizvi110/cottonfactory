...
export class PaymentListComponent implements OnInit {
  ...
  displayedColumns: string[] = ['Amount', 'PaymentDate', 'Actions'];
...
Submit(){
 ...
 // if (this.checkRequiredFields()) {
    ...
    debugger
    this.PaymentService.PostSalesListPagination(req).subscribe(      
    )
    this.Datefrom = this.filterForm.controls.DateFrom.value
    this.Dateto = this.filterForm.controls.DateTo.value
    this.PaymentService.$isDataLoaded.subscribe(result => {
      debugger
        //  this.isLoading = false;
       
    
      this.filterdValues = this.filterForm.value
      this.dataSource = new MatTableDataSource(result.paymentsModel)
      debugger
      this.totalCount = result.totalCount;
