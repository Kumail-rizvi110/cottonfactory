...
@Component({
  selector: 'app-assetsmaintenance-list',
  templateUrl: './assetsmaintenance-list.component.html',
  styleUrls: ['./assetsmaintenance-list.component.css']
})
export class AssetsmaintenanceListComponent implements OnInit {
  ...
  displayedColumns: string[] = ['Cost','MaintenanceDate','Actions'];
  ...
  delete(id: number){
    ...
    (result: boolean) => {
      if (result) {
        this.assetsmaintenanceService.Delete(id).subscribe(res => {
...
Submit(){
 ...
 // if (this.checkRequiredFields()) {
    ...
    debugger
    this.assetsmaintenanceService.PostSalesListPagination(req).subscribe(      
    )
    this.Datefrom = this.filterForm.controls.DateFrom.value
    this.Dateto = this.filterForm.controls.DateTo.value
    this.assetsmaintenanceService.$isDataLoaded.subscribe(result => {
      debugger
      console.log(result)
      ...
      this.dataSource = new MatTableDataSource(result.assetsmaintenanceModel)
      debugger
