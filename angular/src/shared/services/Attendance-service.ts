...
export class AttendanceService {
  ...
  constructor(private http: HttpClient) { }
   ...
   create(body: AttendanceDto | undefined) : Observable<any> {
     debugger
    const content_ = JSON.stringify(body);
      ...
      return this.http.post<any>(this.REST_API_SERVER + '/api/services/app/Attendance/Create',content_, options_) 
    }
  ...
  Delete(id) { 
    ...
    let options = {headers : headers};
    return this.http.delete<any>(this.REST_API_SERVER +'/api/services/app/Attendance/Delete?id='+id,options)
    .pipe(map(res => {
      this.$isDataLoaded.emit(res['result']);
     }));
  }
